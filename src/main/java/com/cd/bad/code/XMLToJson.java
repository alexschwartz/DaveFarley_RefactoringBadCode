 ...
 * folder type only seen as "history" in toc,why ??do we use other ones? -- it
 * only has one child
 * doc type only seen two "tr" and "history" in toc??-- it only has one child
 * node with "folder type="history"" is the only child of it parent either "<doc
 * type="tr" key="xxx" trnum="xxx"....." or "<doc type="tr" trnum="xxx""
 * <folder type="history" could has more then one doc children .e.g.
 * title="History of AMM31-32-00-720-807" in 700/amm
 *
 *
 *
 * folder element has the follwing to identify itself:
 * 1, key
 * 2, type="history", in this case, folder is the only child of doc element with
 * type ="tr"?????
 *
...
public class XMLToJson {
     ...
     * <folder key="LOETR" type="loetr" title="List of Effective TRs"
     * file="loetr.html">
     * <doc type="tr" trnum="TR12-118" trdate="May 07/2012"
     ...
     * file="TR12-118.pdf" refloc="AMM12-31-00-660-806">
     ...
     * if the inner key fully include the previous key, omit it, otherwise use full
     * string
     * the xpath string should be:
